...
**Golden dataset:** Six cases (ids 1–6) with canonical `text_chunk` (draft) + `prd_context` in `test_cases`. `seed_data()` **upserts** those rows on every API startup (metadata + baseline copy). User-edited PRD/draft live in **`golden_case_content_overrides`** (same `test_case_id`); `fetch_test_case_by_id` **merges** override over `test_cases` when a row exists. `seed_data()` does **not** touch overrides, so edits survive API restarts. Mirror in `frontend/lib/constants.ts` (`TEST_CASES`) for labels, steps, architecture contrast, and `expectedVerdict`; **Evaluation** hydrates PRD/draft from **`GET /api/test-cases/{id}`** (merged) and saves with **`PUT /api/test-cases/{id}/golden-content`** JSON `{ prd_context, text_chunk }`.
...
**Multi-change (one POST):** `EvaluateBody` may set **`temperature` + `system_prompt?` + `prd_context?` + `shadow_mode` + `model?` together**. Optional header **`X-OpenAI-API-Key`**. Effective PRD = non-empty body `prd_context` → else merged SQLite row (`test_cases` + optional override) → else sentinel. **`text_chunk` (draft) always comes from the merged DB row for `{id}`** — there is no body override for draft. **GET** only varies query `temperature` / `shadow_mode`; system prompt is always `SYSTEM_PROMPT`, PRD/draft from merged DB; no per-request model/key override on GET.
...
**Multi-change (repo):** Changing canonical golden **titles/steps/expected labels** still requires **`frontend/lib/constants.ts` and `backend/database.py` `_SEED_ROWS`** in sync; restart API so `seed_data()` upserts ids 1–6. Changing **PRD/draft text** for runs is done in the UI (saved to **`golden_case_content_overrides`**) or by calling **`PUT /api/test-cases/{id}/golden-content`**.

**Invariant:** `calibration_router` always uses **LLM verdict + confidence before readability merge**, regardless of knob combination.

**DB:** `knowledge_base.db` — `test_cases` (6 golden rows + optional custom ids), **`golden_case_content_overrides`** (user PRD + draft per case id), `evaluation_logs` (+ `evidence_quote`, etc.). `GET /api/anti-patterns` = recent FAIL/WARNING.

**UI (Live Demo):** sidebar golden list (expected verdict badges) · tabs Evaluation / Prompt Engineering / Anti-Pattern **/ Data Contracts** · header **Runs** (10/100/1000) · **A/B** toggle · **Config OpenAI key** (modal) · Evaluation: large case title; **Steps + Expected** single column; **PRD context** and **documentation draft** panels with **Edit / Save / Cancel** (persist via `PUT …/golden-content`, then runs use merged DB text + POST `prd_context`); per-Variant **OpenAI model** + temperature · N× stability heatmap + score (match % + **WARNING/FAIL/other** remainder) · **collapsible** evaluation trace (`<details>`) · Publish placeholder. **Data Contracts** tab documents `QAEvaluationResult` + notes on merged SSE payload.
